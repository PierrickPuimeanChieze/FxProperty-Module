 ...
 * @author lahvac
 */
public class AddFXPropertyCodeGenerator implements CodeGenerator {
    ...
    public AddFXPropertyCodeGenerator(JTextComponent component, String className, List<String> existingFields, String[] pcsName, String[] vcsName) {
    ...
    public String getDisplayName() {
        return NbBundle.getMessage(AddFXPropertyCodeGenerator.class, "DN_AddFXProperty");
    }
    ...
    public void perform(FileObject file, JTextComponent pane) {
        JButton ok = new JButton(NbBundle.getMessage(AddFXPropertyCodeGenerator.class, "LBL_ButtonOK"));
        final AddFXPropertyPanel addPropertyPanel = new AddFXPropertyPanel(file, className, existingFields, pcsName, vcsName, ok);
        String caption = NbBundle.getMessage(AddFXPropertyCodeGenerator.class, "CAP_AddFXProperty");
        String cancel = NbBundle.getMessage(AddFXPropertyCodeGenerator.class, "LBL_ButtonCancel");
     ...
     */
    static void insertCode2(final FileObject file, final JTextComponent pane, final AddFXPropertyConfig config) {
            final Document doc = pane.getDocument();
            final Reformat r = Reformat.get(pane.getDocument());
            final String code = new AddFXPropertyGenerator().generate(config);
    ...
    static void insertCode(final FileObject file, final JTextComponent pane, final AddFXPropertyConfig config) {
        try {
    ...
    static void insertCodeImpl(final FileObject file, final JTextComponent pane, final AddFXPropertyConfig config) {
        try {
            final Document doc = pane.getDocument();
            final Reformat r = Reformat.get(pane.getDocument());
            ...
            try {
                NbDocument.runAtomicAsUser((StyledDocument) doc, new Runnable() {
                    public void run() {
                        try {
                            String code = new AddFXPropertyGenerator().generate(config);
                            int startOffset = pane.getCaretPosition();
    ...
    public static final class Factory implements CodeGenerator.Factory {
        ...
        public List<? extends CodeGenerator> create(Lookup context) {
            ...
            return Collections.singletonList(new AddFXPropertyCodeGenerator(component, className, existingFields, pcsName,vcsName));
        }
